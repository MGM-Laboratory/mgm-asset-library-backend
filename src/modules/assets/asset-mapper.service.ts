...
import { LocalizedJson, pickTranslation, resolveLocalized } from '../../common/i18n/locale-resolver';
...
export class AssetMapperService {
  ...
  async toSummary(asset: AssetWithRelations, locale: Locale): Promise<AssetSummaryDto> {
    ...
    return {
      ...
      categoryName: resolveLocalized(asset.category.name as LocalizedJson, locale) ?? asset.category.slug,
  ...
  async toDetail(asset: AssetWithRelations, ctx: MapperContext): Promise<AssetDetailDto> {
    ...
    const canEdit = !!ctx.requester && (ctx.requester.id === asset.ownerId || ctx.requester.isAdmin);
    return {
      id: asset.id,
      slug: asset.slug,
      title: asset.title,
      ...
      category: {
        ...
        name: resolveLocalized(asset.category.name as LocalizedJson, ctx.locale) ?? asset.category.slug,
      },
      license: { id: asset.license.id, slug: asset.license.slug, name: asset.license.name },
      tags: asset.tags.map((t) => ({ id: t.tag.id, slug: t.tag.slug, displayName: t.tag.displayName })),
