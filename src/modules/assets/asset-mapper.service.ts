...
export class AssetMapperService {
  ...
  async toDetail(asset: AssetWithRelations, ctx: MapperContext): Promise<AssetDetailDto> {
    ...
    // Re-sign every preview-media item's viewUrl on read so the link the
    // client sees is always fresh (the bucket-side TTL is short).
      ...
      ? await Promise.all(
          previewMediaRaw
            ...
            .map(async (m) => ({
              ...
              viewUrl: await this.s3.presignGet('editor', String(m.key)),
            })),
        )
      : [];
